# 交接文档：qq-farm-bot（更新 2026-08-25，普通偷菜按成熟点唤醒并与施肥监控解耦）
...
- **普通偷菜与施肥监控必须解耦**：普通好友有已知成熟墙钟后只在成熟点唤醒，不得因为进入 66/122 分钟观察窗口就做 3–5 秒或 15–30 秒全好友摘要重查；完全没有任何成熟墙钟时只允许 5–8 分钟低频重新发现。重点好友分钟级基线、施肥 HOT 秒级目标追踪和自然成熟 PREARM 继续走各自独立时钟，不能扩散成普通偷菜轮询。
...
- **每轮改动测试通过后必须上传 GitHub**：提交到当前仓库并通过 SSH 推送 `origin/main`；推送前确认 `core/data/`、日志、账号凭据、Webhook/Token 等敏感数据未被跟踪。推送失败必须明确报告，不得声称已完成。
- **GitHub 零个人信息硬门**：受 Git 跟踪的代码、文档、提交标题、文件名不得包含账号/好友昵称或 GID、服务器用户名、机器绝对路径、内网地址、个人邮箱、Webhook、API Key、Token、Cookie、签名 URL 或登录信息。生产证据只能写匿名结论（如“账号 A”“重点好友 A”），绝不能复制原始身份字段。任何自动进化提交都必须先通过父进程隐私扫描，Agent 自己禁止 `git push`。
...
- 生效逻辑（worker）：
  ...
  - **主动巡田（2026-08-24 降频）**：wx 的 GetAll 好友摘要根本不带 `ripe_time_sec`（实测：好友有在长的作物摘要也显示「暂无成熟时刻」），所以仍需 `watchlistPollTick` 只对重点好友进门建立地块基线。但普通收菜、重种等变化无关紧要：>122 分钟/未知/无作物改为 **5-8 分钟一次**，≤122 分钟也只需 **2-4 分钟一次**，≤60 秒交 PREARM 的成熟墙钟到点抢收；只有 `fertilizer-watch` 确认施肥证据才切秒级 HOT。异常降速时，非 HOT 的重点巡田在原分钟级间隔上再追加约 90-135 秒，HOT 不降速；不再等待长冷却，也不会产生 `Enter cooldown`。进门仍喂 `inspectFriendLands` 和精确 ripeAt（并入 `getNextWatchlistStealDueAtMs`）
  - `friend-orchestrator.js`：重点好友 PREARM 窗口 10s→60s（`DUE_PREARM_WATCHLIST_MS`）；stealTargets 排序重点最前
  - **122 分钟低频观察窗口**：化肥一次最多催熟约 2 小时，所以重点好友成熟前 122 分钟进入低频观察（普通好友仍 `wakeBeforeMinutes` 默认 66）。它只影响长睡唤醒、摘要刷新和分钟级基线检查，**不会放宽请求治理器的 Enter/AllLands 门限**；仅当成熟墙钟提前、施肥次数下降、`is_nudged` 上升等证据触发 HOT 时，治理器才在最长 45 秒的施肥窗口内临时放宽
...
- **异常分级降速**：15 分钟内非竞速失败 ≥12 次 → 随机 5-10 分钟降速提示。请求出口不再返回 `cooldown`，不会整号暂停；普通农场/帮助 tick 放缓，非 HOT 重点巡田在原间隔上再追加约 90-135 秒。自己收获、好友成熟抢收、施肥 HOT、心跳/登录继续运行
...
- **不能为了检测普通好友施肥而恢复 horizon 分档轮询**：普通好友不是施肥重点对象；需要施肥识别的用户加入重点监控，由独立分钟级基线和 HOT 负责。`wakeBeforeMinutes/watchlistWakeBeforeMinutes` 不得再次进入普通 `armStealWake()` 的重查间隔计算。
