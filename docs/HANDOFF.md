...
- 公开项目发现固定包含 `xxxscarlxrd404/qq-farm-bot` 与 `liyangpengs/qq-farm-bot`，每日至少尝试六组关键词并发现新候选，比较候选 SHA 变化；当日检索结果可复用，但次日仍重新发现新项目。查询有请求、响应大小、时间和候选上限，403/429、超时或部分失败如实保留状态。只记录合法仓库名、SHA、时间和固定状态，不保存任意 API 原文或外部描述，不携带本机认证。
