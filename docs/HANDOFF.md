# 交接文档：qq-farm-bot（更新 2026-08-26，活动 Agent 负责道具、玩法与专属前端的端到端进化）
...
- Node 20 新活动定向测试 **3/3**、`core/test/*.test.js` 全量 **321/321** 通过；前端 Node 20 类型检查与生产构建通过，仅保留既有的两个 UnoCSS 缺失图标提示。逐项确认未修改自己成熟前 10 秒预留与 30–80ms Harvest、好友到点偷菜、重点用户 HOT/PREARM、请求治理硬预算/普通降速、登录保活、设备协议、网络及 TSDK/ACE。
- 本轮不重启 Bot、不推送远端。回滚使用 `git revert <本轮提交>`；若以后应用或回滚，只能复用既有 `farm:0.0`。回滚会移除“雨落成诗”专属只读卡片并恢复过期鹊桥活动页/Worker 死分支，但不得借机恢复已停用的青梅/鹊桥配置键或改动核心收益链。
