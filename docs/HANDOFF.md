...
## 沉淀记录（2026-09-26，维护会话）

- 新增私有 skill `core/docs/skills/farm-activity-seed-repair/SKILL.md`（活动新种子/背包未知ID/seedRecognition 缺口/缺图的取证登记闭环，含月下美人与枸杞两轮案例要点），已入本机 skill 索引；纯文档沉淀，零生产代码改动。


### 在线捣乱与重点监控解耦 + 全好友观察层（2026-09-26 用户最终定标）

- 用户撤销"在线捣乱必须以重点监控为前提"：在线检测面向全部好友，在线捣乱按每个好友独立开关，与重点监控无依赖；重点只做优先/加速，不是前提。本轮已回退门槛补丁（未发布），保留独立小修：toggle 路由对旧配置 gid 的 Number 归一化去重、"已开启捣乱"准确文案、friend store toggleAutoBad 迟到响应账号隔离（stale 不写名单不弹错）、`friend-auto-bad.test.js` FARM_DATA_DIR 临时目录隔离。
- 全好友观察层（`friend-auto-bad.js` runObservePass，二轮审查定稿）：对全部好友做**限速只读采样**——`visitFriendForAutoBad` `allowPlace:false` 纯 Enter→presence→Leave 零写，Enter 回包经 noteEnterPresence 入 friend-activity，证据唤醒 autoBad 目标即时出手（不排队尾）。**盲探测全局派发槽**：所有无新鲜在线证据、需先 Enter 探明的探测（动作档+观察层）在实际发起处共用 5s 硬间隔槽，一次 tick 至多一个盲 Enter（批量到期/初始列表/失败重排都不突发）；已知新鲜证据的动作唤醒优先不占盲槽但仍串行/每目标节奏/退避。名册三态契约（第三轮定稿）：friend-api 新增只读名册快照——`recordRosterReply` 只在 `getAllFriends` 的三个真实成功出口（QQ GetGameFriends/QQ SyncAll+GetAll 兜底/WeChat GetAll）记录 `{gids, at}`，失败路径 throw 不写快照，调用方据此区分"上次真实成功快照沿用"与"本次失败退避"（不把旧 UI 缓存当刷新成功）；[]=权威空表（好友删净）停探测；任何后台 getAllFriends 成功即时更新快照；序号只和最新**已成功发布**序号比较（latestSuccessfulSeq）——更新的请求失败不消耗序号，旧请求随后合法成功仍可发布，新成功空表也不被旧成功全表覆盖。观察层自带刷新 30min 有界、失败 5min 退避，不再依赖 land-analyzer UI 缓存。跨组公平（第三轮）：动作档总在观察前跑，多个持续离线 autoBad 目标会占尽 5s 盲槽——每最多 2 次盲动作探测后下一个盲槽让给普通名册观察（观察派发时清零计数；名册为空不启用；已知在线证据的动作不占盲槽也不受限）；盲槽被占时动作循环 continue 而非 break，排在后面的已知在线目标同拍仍可动作。游标用稳定 gid 后继（动作档与观察层各一），名单重排/新增低 ID/删除当前 ID 不饿死任何好友；既有巡查/快档 10s 内新鲜在线证据（deps.online）的目标观察层跳过不重复 Enter；10s 内已有真实观测跳过且不伪更新观测时刻；观察不改动作档 done/failCount；观察独立 makeObserveGuard（不含写侧条件，Enter 在途可拦 stop/断线/暂停/黑名单变更），Leave 由 finally 保证。共同守卫（connected/全局暂停/quiet/巡查互斥/让步）拦观察；badRemaining（每日放虫草次数，非通信预算）/badPaused/名单空/done 只拦动作档，观察继续（零写）。tickDelayMs 有界计算：槽空闲/新鲜窗口过期即近拍重试，不空转 30s 也不把未探测当成功长睡。每次真实观察派发记一条固定 event `friend_observe_sample`（friendGid/atHomeSeen/atHome/onlineEvidence/reason/sampledAt/accountId；单条文案带 GID，缺 at_home 字段="未返回在场标志"、显式 false="本次未取得在场证据"、atHome=null 表示未解码不伪称在场/不在场，onlineEvidence 为合并口径独立显示，均不声称离线；≤1 条/5s 槽），用户手机自测可凭日志确认采样确实发生。stop 清全部观察状态，代次栅栏拦在途迟到写。badRemaining 是每日放虫草写额度只限写，读写通信量均仍受既有网络治理层约束。
- 诚实边界：这是"持续运行的限速采样"，不是全好友实时上线广播。5s 是盲探测槽的**最小派发间隔（理论下限）**，不是"每人 5s"或"一圈严格 N×5s"——实际一圈更长：采样受通信预算、全局串行（动作/观察共用派发）、以及到点收获/抢收让路影响，不承诺好友瞬时上线即被发现。独立 server 上线广播（HostAtHomeNotify 等）仍无证实的结构/归属/订阅范围，未实现独立全好友登录广播；现有可信证据（at_home/lands_push/presence_online）仍走 trigger 唤醒。请求成本：一次采样约 2 个已知 RPC（Enter+Leave），理想上限约 24 采样请求/分钟（实际更低，另有既有巡查与动作请求）。真实名册数在运行日志可见（匿名计数）。
- **采样单目标语义澄清**：观察层一次只采样**一个目标**（游标逐个推进），不是全体好友批量下线式轮询；单个目标的 Enter 回包只代表该目标本次在场证据。`friend_observe_sample` 事件里三个证据字段必须分开读：`atHomeSeen` = 回包是否存在 at_home 字段（存在性）；`atHome` = 该字段解码后的真值（null 表示未解码，不伪称在场/不在场）；`onlineEvidence` = 综合合并口径（含巡查/快档/推送等多来源），三者不可互相替代，任何一个缺失都不等于离线。
- **status 日志展示过滤与在线证据独立订阅修复**：status 通道的日志展示过滤此前会让在线证据事件随展示条件被过滤掉；本轮改为展示过滤与在线证据订阅相互独立——即使面板未开启相关日志展示，Friends 页面的在线标记仍能收到证据并刷新。真实 Friends 页面场景已覆盖**不开日志展示时的证据分发**与**页面卸载时的退订**两条路径；日志满 1000 条等长回绕仍正常消费。日志事件携带运行时观察账号与目标 GID（单条文案含 GID），公共文档不出现任何实际账号身份。
- **主协调真实验证收口（本轮发布门）**：Node 20 全量后端 **814/814 通过（0 fail / 0 skip）**；`vue-tsc -b` 0 error；隔离 vite 生产构建通过；改动文件 lint 零新增错误（friend-api 六条原有 error 保留）；文本新增行隐私审计通过。逻辑指纹 `8b79e38cfa0c9ab870c6cba10db45d1a24c543f2b5aa931120c7a82da2c0d082`（原始本机报告 tmp/all-online-coordinator-validation.json）。**独立旧基线 5a3cb15 反向对照**（真实 runTickBody，产线源 hash 一致，见 tmp/all-online-counterexample.json）：捣乱名单空、写额度 0 的场景下，旧版普通好友 **0 次采样**，新版 2 名普通好友**各 1 次**且 `allowPlace:false` 零写——证明全好友观察确为新行为，非"新函数不存在"式冒充。本轮不声称真实手机即时识别或草虫端到端成功。

### 纯被动证据触发：观察层/盲探测全撤 + lands_push 剔除在线源（2026-09-26 用户最终定标，覆盖上方"全好友观察层"轮）

- **现行要求（以此为准，上文"全好友观察层/10-15s 盲探测/5-8min done 探测"均为历史）**：在线识别是被动 trigger——只消费既有可信在线事件（at_home / presence_online；批量 presence 轮询保持禁用）。`friend-auto-bad.js` 已删除 runObservePass、自动名册拉取、friend_observe_sample 与全部无证据盲探测：无新鲜在线证据即零 Enter/Leave/GetAll，启停/新名单/时间推进不制造请求。证据经 friend-activity `onOnlineEvidence` 唤醒（合并最早已排程唤醒，连续事件不互相推迟；done 只清离场观测不空唤醒）；发起前复验 10s 新鲜窗口（守卫内亦含 connected+新鲜证据复查）；真实派发错峰 ≥2s 有界。
- **离场复位不凭沉默**：订阅 friend-activity `onPresenceObservation`（纯内存，消费既有进门回包 noteEnterPresence；缺 at_home 字段不当明确 false、完全不推进）。done 复位仅两条确证路径：两次显式 at_home=false 观测跨度 ≥3 分钟，或服务端 last_online 确证 ≥3 分钟且晚于 doneAt；单次 false+沉默不复位；在线证据清首见重算。
- **农场变化 ≠ 主人上线（协议审查定论）**：LandsNotify 推送 proto 只有 lands 与 host_gid，无操作者/主人在线字段，且可由被放虫/放草/偷菜触发——`lands_push` 已从在线源剔除（core friend-activity ONLINE_SOURCES 与 web friend.ts 在线白名单同步），保留为普通活跃/地块变化证据与收菜快通道，不改 network/worker 收益调度。
- **运行时昵称**：friend-activity 只读内存名册（有界 200，remark>name 的好友列表快照 + EnterReply.basic.name 喂入，零补名请求）；展示名解析 `resolveFriendDisplayName` 识别 `GID:x` 占位并优先刚取得的真昵称；friend_activity_evidence 文案与 autoBad 完成/复位日志、上线提示带昵称，删除 [重点] 误导前缀。公开文档/测试只用合成名。
- 收尾修复：presence 订阅异常日志固定原因码（不拼 err.message）；friend-api 六条历史 lint error（catch(_) / forceRefresh 未用）顺手清零。
- 排程统一：auto_bad 全部定时（start/tick finally/证据唤醒）收敛到 `schedulePoll`（维护真实到期时刻 `pollTimerAt`，只提前不推迟，回调运行即清标记，stop 清零）；`tickDelayMs` 对已到期且证据仍新鲜的目标按 dispatchGap/≥500ms 有效 deadline 醒，不再吞掉更早唤醒或睡满 30s（测试 core/test/friend-auto-bad-wake-vm.test.js）。
- **主协调真实验证收口（本轮发布门，2026-09-26）**：Node 20 全量后端 **809/809 通过（fail/cancel/skip 全 0）**；`vue-tsc -b` 与隔离生产构建通过；改动文件 lint 0 error；新增行隐私审计通过。逻辑指纹 `38e3e20cf3d7c8f7d3cdba2261f49a1395157578f28bc932a68e44f9692b32af`（本机原始记录 tmp/trigger-only-coordinator-validation.json）。**独立旧基线 bf8d5c3 反向对照**：模拟 20 分钟无在线证据，旧版普通好友与已选捣乱目标两场景各 120 次访问，新版两场景均 0；隔离真实 friend-activity 执行相同 lands_push，旧版 online=true、新版 online=false 而 activity 仍保留；不涉及真实游戏请求（本机证据 tmp/trigger-only-counterexample.json，源码哈希一致）。新增 timer 竞争已通过真实生产调度体的假时钟回归；不引用子 Agent 在共享目录 stash 的自报反例作为协调验证。
- 只读研究采用 2 路独立核查 + Claude Code 3 个子 Agent，协议/历史部分已完成、公开项目广搜部分完成；**尚未证实全好友独立登录广播**——不把外部注释上升为"某平台永远不支持"，也不把单农场驻留变化当作主人登录推送。私有主审报告 tmp/presence-research-reviewed.md，外部地址/身份不进公共文档。昵称来自运行时缓存；真实手机即时上线与真实草虫结果未宣称通过。
