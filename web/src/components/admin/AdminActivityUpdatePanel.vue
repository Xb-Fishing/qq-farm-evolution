...
const ACTIVITY_RULE_INSIGHT_DEFINITIONS: Array<Omit<ActivityRuleInsight, 'evidence'> & { pattern: RegExp }> = [
  { key: 'bear-grow', title: '比熊领养与投喂成长', description: '已接入流程卡；投喂、领养与领取永久比熊已开放面板手动操作，成长进度实时状态当前快照未展示、由操作前校验读取，元气糕库存随背包读取展示。', kind: 'gameplay', pattern: /投喂.*培育比熊|投喂.*比熊幼崽/ },
  { key: 'bear-care', title: '看护与比熊变异', description: '已接入看护条件；商城与玩法说明的产量/售价差异单独提示，当前看护状态待确认。', kind: 'gameplay', pattern: /比熊处于看护状态/ },
  { key: 'bear-treasure', title: '成年寻宝', description: '已接入消耗与奖励关系；寻宝已开放面板手动操作，剩余次数与单次消耗由操作前校验读取、当前快照未展示，元气糕库存随背包读取展示。', kind: 'gameplay', pattern: /寻宝玩法/ },
  { key: 'bear-escort', title: '宝藏护送与结算', description: '已接入自动护送与结束条件；倒计时/资金状态待确认，350/400 初始价值存在说明差异。', kind: 'gameplay', pattern: /宝藏护送/ },
  { key: 'bear-raid', title: '好友夺宝与挑战书', description: '已接入三档收益、每日上限和目标条件；挑战书库存随背包读取展示，今日次数与目标护送状态待官方字段证据，好友交互操作暂未开放。', kind: 'gameplay', pattern: /夺宝博弈|夺宝条件/ },
  { key: 'bear-pity', title: '骰子胜负与安慰礼', description: '已接入胜负分配和连续失败保底；安慰礼领取已开放面板手动操作，可领补偿数量由操作前校验读取、当前快照未展示。', kind: 'gameplay', pattern: /胜负与保底|夺宝安慰礼/ },
  { key: 'bear-album', title: '爪印手记与故事奖励', description: '已接入解锁与领奖流程；手记奖励领取已开放面板手动操作，解锁与领取状态由操作前校验读取、当前快照未展示，field 110 奖励记录不可直接认定为手记。', kind: 'gameplay', pattern: /爪印手记.*照片墙|每解锁一则手记/ },
  { key: 'bear-tactics', title: '锦囊选择与刷新', description: '已接入五类锦囊及免费/付费刷新边界；2/3 个每日锦囊有说明差异，当前次数待确认。', kind: 'gameplay', pattern: /锦囊系统/ },
  { key: 'bear-shop', title: '幸运星游记商城', description: '已接入商品、价格、库存与原始状态码；兑换能力已具备但面板暂无商品兑换入口，状态码语义待官方样本。', kind: 'gameplay', pattern: /幸运星.*兑换.*限定奖励/ },
  { key: 'bear-rank', title: '幸运星好友排名', description: '已接入排名说明；排名当前状态与读取路径待确认。', kind: 'gameplay', pattern: /幸运星排行榜/ },
  { key: 'bear-gift', title: '每日免费稀有种子礼包', description: '已接入每日刷新与累计规则；礼包领取已开放面板手动操作；种子配置已核对狗尾草、芦苇与四格泡泡棉花糖；元气糕为额外掉落道具，继续巡检背包识别缺口。', kind: 'gameplay', pattern: /每日.*赠送.*稀有种子礼包|每日.*免费.*稀有种子礼包/ },
