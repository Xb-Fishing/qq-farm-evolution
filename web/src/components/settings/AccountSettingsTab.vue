        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
          <div class="min-w-0 flex flex-1 items-center gap-3">
            ...
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-0.5 flex flex-wrap items-center gap-1.5">
                ...
                <span
                  v-if="acc.autoLogin === false"
                  class="rounded px-1.5 py-0.5 text-[10px] font-medium leading-tight bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
                >
                  不登录
                </span>
              </div>
          ...
          <div class="flex items-center justify-end gap-2 sm:flex-col sm:items-end">
            <span class="flex items-center gap-1 text-xs text-gray-500 sm:hidden">
              <div class="h-2 w-2 rounded-full" :class="acc.running ? 'bg-green-500' : (acc.autoLogin === false ? 'bg-amber-400' : 'bg-gray-300')" />
              {{ acc.running ? '运行中' : (acc.autoLogin === false ? '不登录' : '已停止') }}
            </span>
            <BaseButton
              variant="secondary"
              size="sm"
              class="border rounded-full shadow-sm transition-all duration-500 ease-in-out active:scale-95"
              :class="[acc.running ? 'sm:w-20 border-red-200 bg-red-50 text-red-600 hover:bg-red-100 focus:ring-red-500 active:border-red-300 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/30 dark:focus:ring-red-500 dark:active:border-red-700' : 'sm:w-24 border-green-200 bg-green-50 text-green-600 hover:bg-green-100 focus:ring-green-500 active:border-green-300 dark:border-green-800 dark:bg-green-900/20 dark:text-green-400 dark:hover:bg-green-900/30 dark:focus:ring-green-500 dark:active:border-green-700']"
              :disabled="!acc.running && isAccountOpsDisabled"
              :title="!acc.running && isAccountOpsDisabled ? '账号已到期，无法启动账号' : ''"
              @click.stop="emit('toggle', acc)"
            >
              <div :class="acc.running ? 'i-carbon-stop-filled' : 'i-carbon-login-filled'" class="mr-1" />
              {{ acc.running ? '停止' : '立即登录' }}
            </BaseButton>
            <BaseButton
              v-if="acc.autoLogin !== false"
              variant="secondary"
              size="sm"
              class="border rounded-full border-amber-200 bg-amber-50 text-amber-700 shadow-sm hover:bg-amber-100 focus:ring-amber-500 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-400 dark:hover:bg-amber-900/30"
              title="停止并禁止自动登录，重启 bot 也不会上号"
              @click.stop="emit('holdOffline', acc)"
            >
              不登录
            </BaseButton>
          </div>
