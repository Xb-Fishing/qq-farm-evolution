...
export function useAutomationSettings({
  ...
  const localAutomationSettings = ref({
    ...
    kickoutRelogin: { delayMinutes: 0, validUntil: '' },
  ...
  function syncLocalAutomationSettings() {
    if (settings.value) {
      ...
      localAutomationSettings.value.kickoutRelogin = {
        delayMinutes: 0,
        validUntil: '',
  ...
  async function saveAutomationSettings() {
    ...
    try {
      ...
      const fullSettings = {
        ...
        kickoutRelogin: localAutomationSettings.value.kickoutRelogin,
  ...
  async function runAutoCodeRefreshNow() {
    ...
    try {
      ...
      if (res.ok)
        showAlert('已手动触发换 Code，账号将重连；在线时无需使用此按钮', 'primary')
      else
