...
- 双 Agent 阶段报错后，主 Agent 根据脱敏失败信息诊断并批准具体文件范围，子 Agent 执行修复，再由主 Agent 验收并重跑原失败阶段；每轮最多处理两次。输出/协作模块可在明确审批后修复，发布器、凭据与隐私检查保持保护。页面显示失败原因、当前处理阶段及次数；需要人工配置或不能安全修复时保留本地状态。
