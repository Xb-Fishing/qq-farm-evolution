...
> [!WARNING]
> 快乐种田，谨慎使用。本项目仅供学习和研究，自动化操作可能违反游戏服务条款，账号及其他相关风险由使用者自行承担。本人目前主要从事具身工作，以前做多模态图像生成，纯喜欢vibe conding，来探索一些未知领域，希望此项目也能激发你的兴趣。
...
Bot 每天北京时间 00:00–01:00 最多自动启动一轮综合进化，将安全巡检与缓存活动增量合并处理；失败或重启也不另起同日自动任务，手动触发仍可独立使用：

- 在「活动中心」顶部或「自动进化 / 活动分析」弹窗配置**双 Agent 协作、主 Agent、子 Agent**，选择后点「保存配置」。关闭双 Agent 时沿用原单执行器流程，旧配置升级后保持原选择。
- 主 Agent 与子 Agent 均由页面选择，职责不绑定具体执行器。子 Agent 承担反馈初检、排查、检索、复现、实现和自检；主 Agent 集中确认方案与最终验收。全量测试通过、主 Agent 最终复核批准后才生成提交；页面显示当前阶段和执行者。
...
- 每日先由所选子 Agent 复盘反馈并准备具体方案；主 Agent 确认范围和验收条件，再由子 Agent 实施，最终主 Agent 验收并提炼可复用经验，经验只在完整审批后保存到本机并用于下一轮提示词。流程 skill 见 `docs/skills/farm-evolution-review/SKILL.md`。
- 每天把面板点击、相关请求结果/耗时、前端错误和运行告警作为巡检反馈；只保存固定类别与匿名关联号，不记录输入内容、身份或凭据。本机日流水保留 24 小时；主 Agent 完成本轮反馈验收且验证/发布成功后，按采样截止时间清除已处理部分，保留进化期间的新事件。页面展示最近 24 小时摘要。
- 双 Agent 首次检查当前逻辑时运行全部已登记回归及前端验证；同一内容指纹已通过就跨日复用，源码、测试或依赖变化后重验。每天新增错误仍需复现和补测试，测试未覆盖的业务路径不能算已验证；不会自动重放线上扣费、领奖等操作。
- 每日公开项目发现读取本机私有参考配置，用包含 qq-farm 的六组关键词按最新更新寻找候选，并采样近期提交评估活跃度。子 Agent 深入只读比较更新内容，主 Agent 核实；限流或不可用时显示未完成部分，不虚构搜索成功。
...
反向对照若属于批准的验收要求，必须登记为 `baselineChecks`。协调进程在隔离副本执行当前代码和旧源码的同组测试，只承认真实行为断言失败；导入、环境错误或子 Agent 自报不能代替验证证据。批准范围内的返工直接交子 Agent，重验后回到最终验收，不再对同一结果连续做修复复核和最终复核。
