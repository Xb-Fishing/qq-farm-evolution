...
- 每日先由主 Agent（推荐 Codex）复盘实际运行反馈、定位问题并分派任务；子 Agent（推荐 Claude）负责检索、复现和实施。最终由主 Agent 验收并提炼可复用经验，经验只在完整审批后保存到本机并用于下一轮提示词。流程 skill 见 `docs/skills/farm-evolution-review/SKILL.md`。
...
- 每日公开项目发现固定检查 `xxxscarlxrd404/qq-farm-bot`、`liyangpengs/qq-farm-bot`，并用六组关键词寻找新候选。子 Agent 深入只读比较更新内容，主 Agent 核实；限流或不可用时显示未完成部分，不虚构搜索成功。
