...
test('two farm accounts get different stable device ids that do not change', () => {
    const first = store.addOrUpdateAccount({ username: 'bob', name: 'Farm A' }).accounts.at(-1);
    const second = store.addOrUpdateAccount({ username: 'bob', name: 'Farm B' }).accounts.at(-1);
    const a = store.getDeviceProtocolForAccount(first.id);
    const b = store.getDeviceProtocolForAccount(second.id);
    assert.equal(a.enabled, true);
    assert.equal(b.enabled, true);
    assert.ok(a.deviceId);
    assert.notEqual(a.deviceId, b.deviceId);
    assert.equal(store.getDeviceProtocolForAccount(first.id).deviceId, a.deviceId);
});
