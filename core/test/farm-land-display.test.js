...
test('land analysis caches every maturity wall clock and all earliest target ids', () => {
  const now = Math.floor(Date.now() / 1000);
  const growingPlant = (matureAt) => ({
    id: 1060032,
    name: '测试作物',
    phases: [
      { phase: 2, begin_time: now - 10 },
      { phase: 6, phase_id: 19, begin_time: matureAt },
    ],
  });
  const analysis = analyzeLands([
    { id: 1, unlocked: true, plant: growingPlant(now + 5) },
    { id: 2, unlocked: true, plant: growingPlant(now + 5) },
    { id: 3, unlocked: true, plant: growingPlant(now + 12) },
    {
      id: 4,
      unlocked: true,
      plant: { id: 1060032, phases: [{ phase: 6, phase_id: 19, begin_time: now - 1 }] },
    },
  ]);

  assert.equal(analysis.nextMatureSec, now + 5);
  assert.deepEqual(analysis.nextMatureLandIds, [1, 2]);
  assert.deepEqual(analysis.matureSchedule, [
    { landId: 1, matureAtSec: now + 5 },
    { landId: 2, matureAtSec: now + 5 },
    { landId: 3, matureAtSec: now + 12 },
    { landId: 4, matureAtSec: now - 1 },
  ]);
});
