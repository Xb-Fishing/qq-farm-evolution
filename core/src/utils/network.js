...
const requestGovernor = require('../services/request-governor');
...
// 风控探针观察：未知推送只记首次类型，不解析、不响应、不重试；上限避免恶意类型刷日志/内存。
const unknownNotifyTypes = new Set();
const MAX_UNKNOWN_NOTIFY_TYPES = 32;
...
function pauseAceReports() {
    if (aceService) aceService.pause();
}

function resumeAceReports() {
    if (aceService) aceService.resume();
}
...
/** Promise 版发送 */
function sendMsgAsync(serviceName, methodName, bodyBytes, timeout = 20000, options = {}) {
    return new Promise((resolve, reject) => {
        // 请求治理：硬预算过闸 + 异常画像（白名单放行；预算拒绝走正常错误路径）。
        // 注意：被预算拦截的请求不会计入失败窗口，避免降速阈值自我强化。
        const gate = requestGovernor.checkRequest(serviceName, methodName);
        if (!gate.allowed) {
            reject(new Error(`请求被治理器拦截(${gate.reason}): ${methodName}`));
            return;
        }
        if (!ws || ws.readyState !== WebSocket.OPEN) {
        ...
        networkScheduler.setTimeoutTask(timeoutKey, timeout, () => {
            ...
            settled = true;
            pendingCallbacks.delete(seq);
            // 连接已断时的超时/失败不计入异常窗口：那是重连伴生失败，不是风控信号
            if (isConnected()) requestGovernor.recordSent(serviceName, methodName, false);
        ...
        sendMsg(serviceName, methodName, bodyBytes, (err, body, meta) => {
            ...
            if (err) {
                if (isConnected()) {
                    // 只允许调用方豁免明确的服务端业务错误。超时、断线、发送失败仍是
                    // 真实通信异常，不能因为探测模式而从异常统计里消失。
                    const breakerExempt = options.breakerExemptBusinessError === true
                        && err.isServerBusinessError === true;
                    requestGovernor.recordSent(
                        serviceName,
                        methodName,
                        false,
                        Date.now(),
                ...
                requestGovernor.recordSent(serviceName, methodName, true);
                resolve({ body, meta });
            }
        }).then((sent) => {
    ...
    try {
        ...
        if (msgType === 2) {
            ...
            if (cb) {
                ...
                if (errorCode !== 0) {
                    const error = new Error(`${meta.service_name}.${meta.method_name} 错误: code=${errorCode} ${meta.error_message || ''}`);
                    error.isServerBusinessError = true;
                    error.serverErrorCode = errorCode;
                    cb(error);
                } else {
...
function handleNotify(msg) {
    ...
    try {
        ...
        // 未识别推送绝不触发新接口或重试，只留一条有界证据给下一轮安全巡检。
        const safeType = String(type || '(empty)')
            .replace(/[^\w.$/-]/g, '?')
            .slice(0, 160);
        if (unknownNotifyTypes.size < MAX_UNKNOWN_NOTIFY_TYPES && !unknownNotifyTypes.has(safeType)) {
            unknownNotifyTypes.add(safeType);
            logWarn('推送', `收到未识别推送类型，仅记录且不响应/不重试: ${safeType}`, {
                module: 'network',
                event: '未知推送风险',
                notifyType: safeType,
                bodyBytes: eventBody ? eventBody.length : 0,
            });
        }
    } catch (e) {
...
module.exports = {
    ...
    getWsErrorState,
    getAceStatus,
    getRequestProfile: requestGovernor.getRequestProfile,
    getBreakerState: requestGovernor.getBreakerState,
    pauseAceReports,
    resumeAceReports,
