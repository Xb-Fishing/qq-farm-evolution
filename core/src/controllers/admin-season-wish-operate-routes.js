 ...
 * 服务端不做自动重试。分享（shareShare）不在白名单：见 season-wish-operate.js。
 */
const { createActivityReadCache, ACTIVITY_UPSTREAM_CACHE_MS } = require('./activity-read-cache');

const { OPERATIONS } = require('../services/season-wish-operate');

const MANUAL_ACTIONS = new Set(Object.keys(OPERATIONS));

function registerAdminSeasonWishOperateRoutes({
  app,
  provider,
  ...
  app.post('/api/activity/season-wish/operate', async (req, res) => {
    ...
    try {
      ...
      const result = await provider.operateSeasonWish(accountId, action, input);
...
module.exports = { registerAdminSeasonWishOperateRoutes };
