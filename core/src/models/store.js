...
const { parsePauseUntilMs, normalizePauseUntil } = require('../utils/pause-until');
...
const DEFAULT_QUIET_HOURS = {
    ...
    maxSleepMinutes: 120,
    wakeBeforeMinutes: 66,
    // 重点监控好友的盯梢窗口（分钟）。化肥一次最多催熟约 2 小时，默认 122；
    // 普通好友用 wakeBeforeMinutes。可独立调大调小
    watchlistWakeBeforeMinutes: 122,
    // 自定义免打扰：到这个时间点之前完全不做任何检测（偷菜/农场/帮助/巡田）
    pauseUntil: ''
};

/** 解析 pauseUntil 为 Date；无效/已过期返回 null */
function parsePauseUntil(raw) {
    const ts = parsePauseUntilMs(raw);
    if (!Number.isFinite(ts)) return null;
    const d = new Date(ts);
    return d.getTime() > Date.now() ? d : null;
}

/** 被顶号后自定义重登延迟；delayMinutes=0 表示按默认递增退避 */
const DEFAULT_KICKOUT_RELOGIN = {
    delayMinutes: 0,
    validUntil: ''
};

function normalizeMaxSleepMinutes(raw, fallback = 120) {
    const n = Number.parseInt(String(raw), 10);
    if (!Number.isFinite(n)) return fallback;
    return Math.max(10, Math.min(12 * 60, n));
}

function normalizeWakeBeforeMinutes(raw, fallback = 66) {
    const n = Number.parseInt(String(raw), 10);
    if (!Number.isFinite(n)) return fallback;
    return Math.max(5, Math.min(180, n));
}

function normalizeWatchlistWakeBeforeMinutes(raw, fallback = 122) {
    const n = Number.parseInt(String(raw), 10);
    if (!Number.isFinite(n)) return fallback;
    return Math.max(5, Math.min(360, n));
}

function normalizeFriendQuietHours(input, prev = DEFAULT_QUIET_HOURS) {
    const src = input && typeof input === 'object' ? input : {};
    const base = prev && typeof prev === 'object' ? prev : DEFAULT_QUIET_HOURS;
    return {
        enabled: src.enabled !== undefined ? !!src.enabled : !!base.enabled,
        start: normalizeTimeString(src.start, base.start || DEFAULT_QUIET_HOURS.start),
        end: normalizeTimeString(src.end, base.end || DEFAULT_QUIET_HOURS.end),
        maxSleepMinutes: normalizeMaxSleepMinutes(
            src.maxSleepMinutes !== undefined ? src.maxSleepMinutes : base.maxSleepMinutes,
            DEFAULT_QUIET_HOURS.maxSleepMinutes
        ),
        wakeBeforeMinutes: normalizeWakeBeforeMinutes(
            src.wakeBeforeMinutes !== undefined ? src.wakeBeforeMinutes : base.wakeBeforeMinutes,
            DEFAULT_QUIET_HOURS.wakeBeforeMinutes
        ),
        watchlistWakeBeforeMinutes: normalizeWatchlistWakeBeforeMinutes(
            src.watchlistWakeBeforeMinutes !== undefined ? src.watchlistWakeBeforeMinutes : base.watchlistWakeBeforeMinutes,
            DEFAULT_QUIET_HOURS.watchlistWakeBeforeMinutes
        ),
        pauseUntil: normalizePauseUntil(src.pauseUntil !== undefined ? src.pauseUntil : (base.pauseUntil || ''))
    };
}

function normalizeKickoutRelogin(input, prev = DEFAULT_KICKOUT_RELOGIN) {
    const src = input && typeof input === 'object' ? input : {};
    const base = prev && typeof prev === 'object' ? prev : DEFAULT_KICKOUT_RELOGIN;
    const delay = Number.parseInt(String(src.delayMinutes !== undefined ? src.delayMinutes : base.delayMinutes), 10);
    return {
        delayMinutes: Number.isFinite(delay) ? Math.max(0, Math.min(4320, delay)) : 0,
        validUntil: String(src.validUntil !== undefined ? src.validUntil : (base.validUntil || '')).trim()
    };
}
...
const DEFAULT_ACCOUNT_CONFIG = {
    ...
    kickoutRelogin: DEFAULT_KICKOUT_RELOGIN,
    knownFriendGids: [],
    friendBlacklist: [],
    watchlistFriendGids: [],
...
function cloneAccountConfig(config = DEFAULT_ACCOUNT_CONFIG) {
    ...
    return {
        ...
        kickoutRelogin: { ...config.kickoutRelogin || DEFAULT_ACCOUNT_CONFIG.kickoutRelogin },
        knownFriendGids,
...
let accountFallbackConfig = (() => {
    ...
    cfg.kickoutRelogin = { ...DEFAULT_ACCOUNT_CONFIG.kickoutRelogin };
    cfg.knownFriendGids = [];
...
function normalizeAccountConfig(raw, fallbackConfig = accountFallbackConfig) {
    ...
    if (input.friendQuietHours && typeof input.friendQuietHours === 'object') {
        cfg.friendQuietHours = normalizeFriendQuietHours(input.friendQuietHours, cfg.friendQuietHours);
    } else {
        cfg.friendQuietHours = normalizeFriendQuietHours(cfg.friendQuietHours);
    }

    // 被顶号后自定义重登延迟
    if (input.kickoutRelogin && typeof input.kickoutRelogin === 'object') {
        cfg.kickoutRelogin = normalizeKickoutRelogin(input.kickoutRelogin, cfg.kickoutRelogin);
    } else {
        cfg.kickoutRelogin = normalizeKickoutRelogin(cfg.kickoutRelogin);
    }
...
function loadGlobalConfig() {
    ...
    try {
        ...
        if (data.accountDeviceProtocols && typeof data.accountDeviceProtocols === 'object') {
            globalConfig.accountDeviceProtocols = {};
            for (const [key, val] of Object.entries(data.accountDeviceProtocols)) {
                if (key && val && val.deviceId) {
                    globalConfig.accountDeviceProtocols[key] = normalizeStoredDeviceProtocol(val);
                }
            }
        }
...
function sanitizeGlobalConfigBeforeSave() {
    ...
    const rawAccountProtocols = globalConfig.accountDeviceProtocols && typeof globalConfig.accountDeviceProtocols === 'object'
        ? globalConfig.accountDeviceProtocols : {};
    const cleanAccountProtocols = {};
    for (const [key, val] of Object.entries(rawAccountProtocols)) {
        const id = String(key || '').trim();
        if (!id || !val || !val.deviceId) continue;
        cleanAccountProtocols[id] = normalizeStoredDeviceProtocol(val);
    }
    globalConfig.accountDeviceProtocols = cleanAccountProtocols;
}
...
function getConfigSnapshot(accountId) {
    ...
    return {
        ...
        intervals: { ...cfg.intervals },
        friendQuietHours: { ...cfg.friendQuietHours },
        kickoutRelogin: { ...cfg.kickoutRelogin },
...
function applyConfigSnapshot(patch = {}, opts = {}) {
    ...
    if (patch.friendQuietHours && typeof patch.friendQuietHours === 'object') {
        cfg.friendQuietHours = normalizeFriendQuietHours(patch.friendQuietHours, cfg.friendQuietHours);
    }
    if (patch.kickoutRelogin && typeof patch.kickoutRelogin === 'object') {
        cfg.kickoutRelogin = normalizeKickoutRelogin(patch.kickoutRelogin, cfg.kickoutRelogin);
    }
...
function getFriendQuietHours(accountId) {
    return normalizeFriendQuietHours(getAccountConfigSnapshot(accountId).friendQuietHours);
}

function getKickoutRelogin(accountId) {
    return normalizeKickoutRelogin(getAccountConfigSnapshot(accountId).kickoutRelogin);
}

/** 免打扰剩余毫秒；未设置/已过期返回 0 */
function getPauseRemainMs(accountId) {
    const until = parsePauseUntil(getFriendQuietHours(accountId).pauseUntil);
    return until ? until.getTime() - Date.now() : 0;
}
...
function getWatchlistFriendGids(accountId) {
    return [...getAccountConfigSnapshot(accountId).watchlistFriendGids || []];
}

function setWatchlistFriendGids(accountId, gids) {
    const base = getAccountConfigSnapshot(accountId);
    const cfg = normalizeAccountConfig(base, accountFallbackConfig);
    cfg.watchlistFriendGids = Array.isArray(gids)
...
function isAccountAutoLogin(account) {
    return !!(account && account.autoLogin !== false);
}
...
function addOrUpdateAccount(account) {
    ...
    if (account.id) {
        ...
        data.accounts.push({
            ...
            wxCredentialExpiresAt: Math.max(0, Number(account.wxCredentialExpiresAt) || 0),
            wxCredentialExpiresIn: Math.max(0, Number(account.wxCredentialExpiresIn) || 0),
            wxRefreshTokenObservedAt: Math.max(0, Number(account.wxRefreshTokenObservedAt) || 0),
            wxCredentialLastSuccessAt: Math.max(0, Number(account.wxCredentialLastSuccessAt) || 0),
...
function normalizeStoredDeviceProtocol(val) {
    return {
        enabled: true,
        userAgent: String(val.userAgent || '').trim(),
        deviceModel: String(val.deviceModel || '').trim(),
        deviceBrand: String(val.deviceBrand || '').trim(),
        deviceMac: String(val.deviceMac || '').trim(),
        deviceId: String(val.deviceId || '').trim(),
        imei: String(val.imei || '').trim()
    };
...
function ensureAccountDeviceProtocol(accountId) {
    const id = String(accountId || '').trim();
    if (!id) return { ...DEFAULT_DEVICE_PROTOCOL };
    if (!globalConfig.accountDeviceProtocols) globalConfig.accountDeviceProtocols = {};
    const existing = globalConfig.accountDeviceProtocols[id];
    if (existing && existing.deviceId) {
        return { ...existing, enabled: true };
    }
    const { buildStableDeviceProtocol } = require('../utils/device-fingerprint');
    const assigned = buildStableDeviceProtocol(id);
    globalConfig.accountDeviceProtocols[id] = assigned;
    saveGlobalConfig();
    return { ...assigned };
}
...
function getDeviceProtocolForAccount(accountId) {
    ...
    if (id) {
        ...
        const userProtocol = username
            ? (globalConfig.userDeviceProtocols && globalConfig.userDeviceProtocols[username])
            : null;
        if (userProtocol && userProtocol.enabled === true && userProtocol.deviceId) {
            return getUserDeviceProtocol(username);
        }
        return ensureAccountDeviceProtocol(id);
    }
