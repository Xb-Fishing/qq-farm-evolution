...
function baseActivity(root, play, activityId, clientUiUid, protobufField, nowSeconds) {
  return {
    ...
    readOnly: true, writeOperationsSupported: false,
    subActivities: [{
    ...
    protocol: {
      ...
      opaqueReadOnlyFields: [protobufField],
...
// 奖励中的“2 种限定种子”当前没有道具 ID、图片、植物或占地证据：
// 不改 EventPlants.json、不读背包、不猜写命令，活动开放后再按下发证据补齐。
function normalizeWishActivity(snapshot, options = {}) {
  ...
  const storageRule = findLine(lines, /存储5日奖励|存储\s*5\s*日奖励/);
  ...
  const guides = [
    ...
    ['rewards', '祈愿奖励内容', rewardRule, ['奖励包括 2 种限定种子', '烟花互动道具与盆栽装扮', '种子 ID 与图片待活动开放后的下发证据'], '奖励道具 ID、数量与领取进度'],
  ...
  return {
    ...
    missingEvidence: [
      '活动说明证明每日祈愿、限定种子/烟花/盆栽奖励、5 日存储与邮件补发；不能据此推导祈愿或领取的 cmd 与请求参数。',
      '2 种限定种子的道具 ID、图片、植物映射与占地（含是否四格）均无证据：不改 EventPlants.json，待活动开放后按下发/背包/土地证据补齐。',
      `field ${WISH_PROTOBUF_FIELD} 仅保留字段形状诊断（wire 2、子字段 varint），业务语义未声明，不解释。`,
...
function normalizeHappyShareActivity(snapshot, options = {}) {
  ...
  return {
    ...
    ruleSections: lines.map((line, index) => ({ index, line })),
    missingEvidence: [
      '活动说明证明快乐值三途径与档位奖励；不能据此推导领取或分享的 cmd 与请求参数。',
      '快乐值与档位奖励的道具 ID、当前进度均无快照字段证据，未知不按 0 处理。',
      `field ${HAPPY_SHARE_PROTOBUF_FIELD} 仅保留字段形状诊断（wire 2、嵌套子消息），业务语义未声明，不解释。`,
      '分享与点击好友链接是官方客户端社交玩法，Bot 不模拟分享、不伪造点击、不代领奖励。',
...
module.exports = {
  WISH_ACTIVITY_ID, WISH_SIGN_ACTIVITY_ID, WISH_CLIENT_UI_UID, WISH_PROTOBUF_FIELD,
