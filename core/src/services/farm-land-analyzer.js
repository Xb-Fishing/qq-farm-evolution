...
function analyzeLands(lands, debug = false) {
  const result = {
    ...
    harvestableInfo: [], nextMatureSec: 0, nextMatureLandIds: [], matureSchedule: []
  };
  ...
  for (const land of lands) {
    ...
    // 最后一条阶段记录是服务端给出的成熟墙钟。成熟和生长中的作物都记录，
    // 这样自己的独立收获定时器可直接缓存目标地块，不必到点后再 AllLands。
    const lastPhase = plant.phases[plant.phases.length - 1];
    const matureAt = toTimeSec(lastPhase && lastPhase.begin_time);
    ...
    // 记录最早的成熟时间（phases 最后一条即成熟阶段），供调度器到点唤醒收获
    if (matureAt > serverTime) {
      result.matureSchedule.push({ landId, matureAtSec: matureAt });
      if (!result.nextMatureSec || matureAt < result.nextMatureSec) {
        result.nextMatureSec = matureAt;
        result.nextMatureLandIds = [landId];
      } else if (matureAt === result.nextMatureSec) {
        result.nextMatureLandIds.push(landId);
      }
    }
