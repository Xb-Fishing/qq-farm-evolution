...
function resolveLicenseSecret(explicitSecret = '') {
  const secret = String(explicitSecret || getPrivateValue('licenseSecret', 'FARM_LICENSE_SECRET')).trim();
  if (!secret) throw new Error('许可证功能未配置 FARM_LICENSE_SECRET 或本地 private-config.json');
  return secret;
}
...
function aesEncrypt(plainText, secret) {
  const aesKey = crypto.scryptSync(resolveLicenseSecret(secret), 'salt', 32);
  const cipher = crypto.createCipheriv('aes-256-cbc', aesKey, AES_IV);
 ...
 */
function generateLicenseKey(machineId, secret = '') {
  const normalizedId = machineId.replace(/-/g, '').toUpperCase();
  const resolvedSecret = resolveLicenseSecret(secret);
  const combined = normalizedId + resolvedSecret;
  const hashInput = aesEncrypt(combined, resolvedSecret);
 ...
 * 验证许可证密钥是否有效
 */
function verifyLicenseKey(machineId, licenseKey, secret = '') {
