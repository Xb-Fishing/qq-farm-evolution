...
7. 没有可靠问题证据、没有明确安全收益，或现有逻辑已经符合要求时，允许完全不改代码、不改 HANDOFF、不生成提交；禁止为了“完成进化”制造改动或只刷巡检记录。
8. 只要实际修改代码，必须同步更新 docs/HANDOFF.md，记录改了什么、踩坑注意点、验证结果、风险边界和回滚方法；全量测试通过后只创建本地提交，由父进程隐私扫描通过后再推送 GitHub。`;
...
5. 在线快照中的 details 用于道具/商店/奖池，discoveryEvidence.protocolShape 用于定位当前 proto 未声明的字段。对未知玩法可以先补“只读解析 + UI 状态”；只有成功请求样本或官方源码足以证明 cmd/参数时才能加自动执行。已有证据足以完成的道具、只读玩法和前端 UI 不得因为另一项写操作待抓包而全部跳过，也不得只改 HANDOFF 后结束。
...
发现以上任一迹象：只记录到 HANDOFF「风险待处理」（附证据），**绝不添加对新接口/新字段的调用，绝不加重试**；若现有代码里有对未知错误码盲目重试的路径，收紧为「未知错误→停手记录，不重试」。
