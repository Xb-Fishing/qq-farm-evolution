...
2a. **Skill 库（用户 2026-09-24 指示）**：每轮开始先读 core/docs/skills/README.md 索引，命中场景按对应 skill 执行（当前有：活动操作协议确认）。协议确认、公开库检索这类调研工作必须用并行子 agent（Task/Agent）执行，主 agent 只做交叉验证与落地。本轮若形成新的可复用流程（新的检索模式、验证手法、适配套路），沉淀为新 skill 文件并登记索引——skill 沉淀算正式产出，与代码改动同等对待：
