...
function createRuntimeState(deps) {
    ...
    function filterLogs(logList, filter = {}) {
        ...
        // 支持逗号分隔多个事件名（同一动作在不同代码路径可能用中英文两种事件名）
        const eventSet = String(filter.event || '').trim()
            ? new Set(String(filter.event).split(',').map(s => s.trim()).filter(Boolean))
            : null;
        ...
        return (logList || []).filter(entry => {
            ...
            if (eventSet && !eventSet.has(String((entry.meta || {}).event || ''))) return false;
